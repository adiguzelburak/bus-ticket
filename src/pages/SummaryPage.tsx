import { useLocation, useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import type { Trip, Passenger } from "../types";
import { createTicketSale, getAgencies } from "../services/api";
import { format } from "date-fns";
import { useState } from "react";
import { CheckCircle2, Printer, Download, Loader2 } from "lucide-react";
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";
import { useQuery } from "@tanstack/react-query";

interface FormData {
  passengers: Passenger[];
  contact: {
    email: string;
    phone: string;
  };
}

interface LocationState {
  trip: Trip;
  selectedSeats: number[];
  totalAmount: number;
  formData: FormData;
}

function SummaryPage() {
  const location = useLocation();
  const navigate = useNavigate();
  const state = location.state as LocationState | null;

  const [isLoading, setIsLoading] = useState(false);
  const [isSuccess, setIsSuccess] = useState(false);
  const [pnr, setPnr] = useState("");

  const { data: agencies } = useQuery({
    queryKey: ["agencies"],
    queryFn: getAgencies,
  });

  const getAgencyName = (id: string) => {
    return agencies?.find((a) => a.id === id)?.name || id;
  };

  const handlePayment = async () => {
    if (!state) return;
    setIsLoading(true);

    try {
      const { trip, selectedSeats, formData } = state;
      const response = await createTicketSale({
        tripId: trip.id,
        seats: selectedSeats,
        contact: formData.contact,
        passengers: formData.passengers,
      });

      if (response.ok) {
        setPnr(response.pnr);
        setIsSuccess(true);
      } else {
        alert("Ödeme sırasında bir hata oluştu: " + response.message);
      }
    } catch (error) {
      console.error("Payment error:", error);
      alert("Ödeme işlemi başarısız oldu.");
    } finally {
      setIsLoading(false);
    }
  };

  const handleDownloadPDF = () => {
    if (!state || !pnr) return;

    const { trip, totalAmount, formData } = state;
    const doc = new jsPDF();

    doc.setFontSize(20);
    doc.text("Bilet Bilgileri", 14, 22);

    doc.setFontSize(10);
    doc.text(`PNR: ${pnr}`, 14, 32);
    doc.text(
      `Olusuturulma Tarihi: ${format(new Date(), "dd.MM.yyyy HH:mm")}`,
      14,
      38
    );

    doc.setFontSize(12);
    doc.text("Sefer Bilgileri", 14, 50);

    const tripData = [
      ["Firma", trip.company],
      ["KALKIS", getAgencyName(trip.from)],
      ["VARIS", getAgencyName(trip.to)],
      ["TARIH", format(new Date(trip.departure), "dd.MM.yyyy HH:mm")],
    ];

    autoTable(doc, {
      startY: 55,
      head: [],
      body: tripData,
      theme: "plain",
      styles: { fontSize: 10, cellPadding: 1 },
      columnStyles: { 0: { fontStyle: "bold", cellWidth: 40 } },
    });

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    doc.text("Yolcu Listesi", 14, (doc as any).lastAutoTable.finalY + 15);

    const passengerData = formData.passengers.map((p) => [
      p.seat.toString(),
      `${p.firstName} ${p.lastName}`,
      p.idNo,
      p.gender === "male" ? "Erkek" : "Kadın",
    ]);

    autoTable(doc, {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      startY: (doc as any).lastAutoTable.finalY + 20,
      head: [["Koltuk", "Ad Soyad", "TCKN", "Cinsiyet"]],
      body: passengerData,
      theme: "striped",
      headStyles: { fillColor: [41, 128, 185] },
    });

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const finalY = (doc as any).lastAutoTable.finalY + 10;
    doc.text(`Toplam Tutar: ${totalAmount} TL`, 14, finalY);

    doc.save(`bilet-${pnr}.pdf`);
  };

  if (!state) {
    return (
      <div className="container mx-auto p-8 text-center">
        <p className="text-red-500 mb-4">
          İşlem bilgileri bulunamadı. Lütfen sefer seçiminden başlayınız.
        </p>
        <Button onClick={() => navigate("/")}>Başa Dön</Button>
      </div>
    );
  }

  const { trip, selectedSeats, totalAmount, formData } = state;

  if (isSuccess) {
    return (
      <div className="container mx-auto px-4 py-12 max-w-3xl text-center">
        <div className="mb-8 flex justify-center">
          <CheckCircle2 className="w-24 h-24 text-green-500" />
        </div>

        <h1 className="text-3xl font-bold text-green-600 mb-4">
          Ödeme Başarılı!
        </h1>
        <p className="text-gray-600 mb-8">
          Biletiniz başarıyla oluşturulmuştur. İyi yolculuklar dileriz.
        </p>

        <Card className="mb-8 text-left">
          <CardHeader className="border-b bg-gray-50">
            <div className="flex justify-between items-center">
              <CardTitle>Bilet Detayları</CardTitle>
              <span className="font-mono text-sm text-gray-500">
                PNR: {pnr}
              </span>
            </div>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                <h3 className="font-semibold text-gray-900 mb-2">
                  Sefer Bilgileri
                </h3>
                <p className="text-sm text-gray-600">
                  <span className="font-medium">Firma:</span> {trip.company}
                </p>
                <p className="text-sm text-gray-600">
                  <span className="font-medium">Güzergah:</span>{" "}
                  {getAgencyName(trip.from)} - {getAgencyName(trip.to)}
                </p>
                <p className="text-sm text-gray-600">
                  <span className="font-medium">Tarih:</span>{" "}
                  {format(new Date(trip.departure), "dd.MM.yyyy HH:mm")}
                </p>
              </div>
              <div>
                <h3 className="font-semibold text-gray-900 mb-2">İletişim</h3>
                <p className="text-sm text-gray-600">
                  {formData.contact.email}
                </p>
                <p className="text-sm text-gray-600">
                  {formData.contact.phone}
                </p>
              </div>
            </div>

            <h3 className="font-semibold text-gray-900 mb-3">Yolcu Listesi</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-100 text-gray-700 uppercase">
                  <tr>
                    <th className="px-4 py-2 rounded-l">Koltuk</th>
                    <th className="px-4 py-2">Ad Soyad</th>
                    <th className="px-4 py-2">TCKN</th>
                    <th className="px-4 py-2 rounded-r">Cinsiyet</th>
                  </tr>
                </thead>
                <tbody>
                  {formData.passengers.map((passenger, idx) => (
                    <tr key={idx} className="border-b">
                      <td className="px-4 py-3 font-medium">
                        {passenger.seat}
                      </td>
                      <td className="px-4 py-3">
                        {passenger.firstName} {passenger.lastName}
                      </td>
                      <td className="px-4 py-3">{passenger.idNo}</td>
                      <td className="px-4 py-3">
                        {passenger.gender === "male" ? "Erkek" : "Kadın"}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>

            <div className="mt-6 flex justify-end items-center border-t pt-4">
              <span className="text-gray-600 mr-4">Toplam Ödenen Tutar:</span>
              <span className="text-2xl font-bold text-blue-600">
                {new Intl.NumberFormat("tr-TR", {
                  style: "currency",
                  currency: "TRY",
                }).format(totalAmount)}
              </span>
            </div>
          </CardContent>
        </Card>

        <div className="flex justify-center gap-4">
          <Button variant="outline" onClick={() => window.print()}>
            <Printer className="mr-2 h-4 w-4" />
            Yazdır
          </Button>
          <Button variant="outline" onClick={handleDownloadPDF}>
            <Download className="mr-2 h-4 w-4" />
            Bilet İndir (PDF)
          </Button>
          <Button onClick={() => navigate("/")}>Yeni Arama Yap</Button>
        </div>
      </div>
    );
  }

  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <h1 className="text-2xl font-bold mb-6">Ödeme ve Onay</h1>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Sefer ve Yolcu Bilgileri</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid gap-4">
                <div className="p-4 bg-gray-50 rounded-lg border">
                  <h3 className="font-semibold mb-2">{trip.company}</h3>
                  <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                    <div>
                      <span className="block font-medium text-gray-900">
                        Kalkış
                      </span>
                      {getAgencyName(trip.from)}
                    </div>
                    <div>
                      <span className="block font-medium text-gray-900">
                        Varış
                      </span>
                      {getAgencyName(trip.to)}
                    </div>
                    <div>
                      <span className="block font-medium text-gray-900">
                        Tarih
                      </span>
                      {format(new Date(trip.departure), "dd.MM.yyyy HH:mm")}
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="font-semibold mb-3">Yolcular</h3>
                  <div className="space-y-3">
                    {formData.passengers.map((p, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between p-3 border rounded hover:bg-gray-50"
                      >
                        <div className="flex items-center gap-3">
                          <span className="bg-blue-100 text-blue-800 text-xs font-bold px-2 py-1 rounded">
                            Koltuk {p.seat}
                          </span>
                          <div>
                            <p className="font-medium">
                              {p.firstName} {p.lastName}
                            </p>
                            <p className="text-xs text-gray-500">
                              TCKN: {p.idNo}
                            </p>
                          </div>
                        </div>
                        <div className="text-sm text-gray-600">
                          {p.gender === "male" ? "Erkek" : "Kadın"}
                        </div>
                      </div>
                    ))}
                  </div>
                </div>

                <div className="pt-4 border-t">
                  <h3 className="font-semibold mb-2">İletişim</h3>
                  <div className="text-sm text-gray-600 grid grid-cols-2 gap-4">
                    <div>
                      <span className="block text-gray-900">E-posta:</span>
                      {formData.contact.email}
                    </div>
                    <div>
                      <span className="block text-gray-900">Telefon:</span>
                      {formData.contact.phone}
                    </div>
                  </div>
                </div>
              </div>
            </CardContent>
          </Card>
        </div>

        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-lg shadow-sm border sticky top-4">
            <h3 className="text-lg font-semibold mb-4">Ödeme Özeti</h3>

            <div className="space-y-3 mb-6">
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">
                  Bilet Fiyatı ({selectedSeats.length} x {trip.price} TL)
                </span>
                <span className="font-medium">
                  {new Intl.NumberFormat("tr-TR", {
                    style: "currency",
                    currency: "TRY",
                  }).format(totalAmount)}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Hizmet Bedeli</span>
                <span className="font-medium">0,00 ₺</span>
              </div>
              <div className="border-t pt-3 mt-3 flex justify-between items-center">
                <span className="font-bold text-gray-900">Toplam Tutar</span>
                <span className="text-2xl font-bold text-blue-600">
                  {new Intl.NumberFormat("tr-TR", {
                    style: "currency",
                    currency: "TRY",
                  }).format(totalAmount)}
                </span>
              </div>
            </div>

            <Button
              className="w-full"
              size="lg"
              onClick={handlePayment}
              disabled={isLoading}
            >
              {isLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  Ödeme Yapılıyor...
                </>
              ) : (
                "Ödemeyi Tamamla"
              )}
            </Button>

            <p className="text-xs text-gray-500 mt-4 text-center">
              Güvenli ödeme altyapısı ile korunmaktadır.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}

export default SummaryPage;
